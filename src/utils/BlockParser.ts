...
export class BlockParser {
  ...
  static parseBlocks(content: string): BlockInfo[] {
    ...
    const newSplitterPositions = BlockParser.findAllSplitterPositions(content, BlockParser.NEW_SPLITTER);
    const oldSplitterPositions = BlockParser.findAllSplitterPositions(content, BlockParser.OLD_SPLITTER);
   ...
   */
  private static removeDuplicatePositions(splitters: Array<{ position: number; splitter: string }>): Array<{ position: number; splitter: string }> {
  ...
  private static reconstructBlocksToString(originalContent: string, blocks: BlockInfo[]): string {
    ...
    for (let i = 1; i < blocks.length; i++) {
      ...
      const betweenContent = originalContent.substring(prevBlock.endPosition, currentBlock.startPosition);
